  ...
  <div>
    ...
    <el-form :inline="true" :model="queryForm">
      ...
      <el-form-item label="费用单号:">
        <el-select v-model="queryForm.orderCode" filterable placeholder="请选择费用单号" clearable>
    ...
    <el-table v-loading="$store.getters.loading" :data="expenses" border tooltip-effect="dark" show-summary :summary-method="getSummaries">
      ...
      <el-table-column align="center" label="费用单号" show-overflow-tooltip prop="orderCode" />
      ...
      <el-table-column align="center" label="操作" width="150">
        <template slot-scope="scope">
          ...
          <el-button type="text" @click="deleteExpense(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
    ...
    <dialogs v-model="dialogVisible" :title="'费用管理表单'" @submitClick="saveExpense">
      <el-form ref="expenseForm" :model="expenseForm" label-width="100px" :rules="rules">
        ...
        <el-form-item label="申请人:" prop="createUser">
          <el-input v-model.trim="expenseForm.createUser" clearable placeholder="请输入申请人" />
        </el-form-item>
        ...
        <el-form-item label="费用单号:" prop="orderCode">
          <el-select v-model="expenseForm.orderCode" filterable placeholder="请选择费用单号">
            <el-option v-for="item in orderOptions" :key="item.code" :label="item.code" :value="item.code" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="费用单付款方式:" prop="paymentWay">
  ...
  data () {
    return {
      ...
      rules: {
        ...
        orderCode: [
          { required: true, message: '请选择费用单编号', trigger: 'change' }
        ],
        price: [
          { required: true, message: '请输入费用单金额', trigger: 'blur' }
        ],
        paymentWay: [
      ...
      this.$refs['expenseForm'].validate((valid) => {
        ...
        if (!params.id) {
          createExpense(params).then(() => {
            ...
            this.$message.success('添加费用单成功')
            this.pageChange(1)
          })
        } else {
          updateExpense(params).then(() => {
            this.dialogVisible = false
            this.$message.success('修改费用单成功')
            this.pageChange(1)
          })
        }
      })
